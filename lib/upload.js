...
function uploadDirectory(aws, bucketName, clientRoot, headerSpec, orderSpec, keyPrefix, sse) {
  ...
  return filesGroupedByOrder.reduce((existingUploads, files) => {
    return existingUploads.then(existingResults => {
      ...
      return Promise.all(
        uploadList.map(u =>
          uploadFile(aws, bucketName, u.filePath, u.fileKey, u.headers, sse, u.contentType)
        )
 ...
 * @param {string} sse - ServerSideEncryption, AES256 | aws:kms | null
 * @param {string} customContentType - Custom content type (optional) text/html | null
 */
function uploadFile(aws, bucketName, filePath, fileKey, headers, sse, customContentType) {
  ...
  const params = {
    ...
    Key: fileKey,
    Body: fileBuffer,
    ContentType: customContentType || mime.getType(filePath)
  };
...
function buildUploadList(files, clientRoot, headerSpec, keyPrefix) {
  ...
  const uploadList = files.map(f => {
    ...
    const normalizedPath = path.normalize(fileRelPath).split(path.sep);
    if (prefix.length) {
      normalizedPath.unshift(...prefix);
    }

    const isHtmlFile = filePath.endsWith('.html');
    const keyPath = normalizedPath.join('/');
    const fileKey =
      isHtmlFile && !filePath.endsWith(DEFAULT_INDEX_FILE) ? removeExtensionPath(keyPath) : keyPath;
    const contentType = isHtmlFile ? HTML_MIME_TYPE : null;
...
const removeExtensionPath = fullpath => {
  let pathWithoutExt = '';
  var lastDotPosition = fullpath.lastIndexOf('.');
  pathWithoutExt = lastDotPosition === -1 ? fullpath : fullpath.substr(0, lastDotPosition);
